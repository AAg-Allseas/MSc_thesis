...
system. Their ability to capture the stochastic dynamics are evaluated with a toy DP model. In this chapter two pilot
tests are performed. The aim of the tests are to test whether the approaches are suitable for a simulating
stochastically excited DP model. The aim is to see whether neural SDEs and DeepONet are capable of capturing or
propagating aleatoric uncertainty. Epistemic uncertainty will not be considered in these tests. 

First \cref{sec:toy_model} will outline the toy model used to generate training data. A description of the system and
included dynamics is given, followed by an overview of the approach used to generate the data. Next in
\cref{sec:pilot_latentSDE}, the latentSDE approach to training a neural SDE is tested. The model architecture is laid
out followed by an overview of the training and results. \Cref{sec:pilot_deeponet} covers the DeepONet pilot test, and
follows the same structure. Finally, the results are compared, and a conclusion is given for the further research in \cref{sec:pilot_conclusion}.
...
performed using PyTorch 2.1.0 with CUDA 12.8. The code used for these pilot tests is available here: \url{https://github.com/AAperghis/thesis_pilot_tests.git}
...
parameters corresponding to the desired response of the ship. These are set to 1 and 0.1 respectively for each degree of
freedom. The setpoint, \(\eta_d\), is set to the origin. No wind feed-forward is used.
...
for the corresponding starting position. The aim of this is to minimise the start-up transient of the simulation. The
residual transient is kept in the training data, as the two approaches should be able to accommodate for this transient
given the initial condition. Furthermore, as the training sets generally consist of subsamples of the entire sequence,
not every training sample includes this transient. 
...
initial tests. The section begins with the network architecture in \cref{subsec:pilot_latentsde_arch}, followed by training procedures and results in \cref{subsec:pilot_latentsde_training}. 

\subsection{Architecture}
\label{subsec:pilot_latentsde_arch}

The overall data flow of the latentSDE approach is shown in \cref{fig:sde_data_flow}. The final model would take model
parameters about the operating conditions as well as some metric describing the weather forecast as input. For the pilot
tests, these input parameters are not included. The output is a time-series of likely motions for that set of
parameters. During training, neither the prior nor the posterior has access to the original force series used to
simulate the motions. The posterior is trained with context provided by the simulated measurements, while the prior is
trained to output a similar statistical distribution as the prior.

\begin{figure}
    \centering
    \includegraphics[page=1]{report/images/diagrams/DataFlow_models.pdf}
    \caption{Data flow in LatentSDE compared to regular time-domain DP simulations}
    \label{fig:sde_data_flow}
\end{figure}
...
\label{subsec:pilot_latentsde_training}
...
\textcite{moyaDeepONetgridUQTrustworthyDeep2023} in their work on uncertainty quantification with DeepONets. The section covers the network architecture in \cref{subsec:pilot_deeponet_arch}, followed by training procedures and results in \cref{subsec:pilot_deeponet_training}.

\subsection{Architecture}
\label{subsec:pilot_deeponet_arch}

The general work flow with the DeepONet based approach is described in \cref{fig:deeponet_data_flow}. In essence, the
model acts exactly as the DP simulation model. It takes as input a time series of a realisation of drift forces and
outputs a series of motions. During training, the same force series realisation is used in the input for both the
simulation model and the DeepONet, and the output series are compared directly.

\begin{figure}
    \centering
    \includegraphics[page=2]{report/images/diagrams/DataFlow_models.pdf}
    \caption{Data flow in DeepONet compared to regular time-domain DP simulations}
    \label{fig:deeponet_data_flow}
\end{figure}
...
\subsection{Training and Evaluation}
\label{subsec:pilot_deeponet_training}
...
\begin{figure}[H]
    ...
    \caption{Randomly selected surge reconstruction of 1 DOF DeepONet.}
...
stochastic force time history as input. In particular, the large, lower frequency motions are captured best.
Some of the smaller,
sharper oscillations are not captured as accurately. This shows the DeepONet approach is feasible for modelling the
dynamics of a stochastically excited dynamic system, thus capturing the aleatoric uncertainty. Inference of the model takes a negligible about of time, marking a
