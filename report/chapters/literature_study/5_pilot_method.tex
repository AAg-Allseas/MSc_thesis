...
\section{DeepONet}
\TODO{Update when tests are actually finished, depending on what (if anything) works}
...
\begin{figure}[H]
    ...
    \caption{MIONet architecture used (3 degrees of freedom)}
...
layer with a kernel size of 2. All the layers used the GELU activation function. In the test case where multiple degrees of freedom were considered, each degree of freedom of the input force was passed
to an independent convolutional network. Each output dimension had an independent tree network with shared branches. 
...
10000 epochs. An initial learning rate of 0.01 was used with a schedule to reduce it on a plateau of test loss. A sample
size of 2048 random timesteps was used to train and test. A 80-20 training testing data split was used. 

DeepONet was first tested with a 1 degree of freedom case. A new dataset was generated from the toy model with pure
surge force and motion. 100 different seeds were used. 

A 3 DOF case was tested with the original dataset. The model struggled to generalise well. The model tended to predict
the mean of all the time-traces per signal before overfitting to the training data. To combat this, dropout was included
in the model architecture, which did improve the performance slightly. 


\section{Conclusion}
Whether due to limitations in the specific architecture used in this work or inherent limitations of the DeepONet framework, the method does
not appear to work as well as neural SDEs for the problem at hand. Furthermore, neural SDEs offer more flexibility for
use in practice, as they do not require a fixed sample length or interval, or generation of a force series for training.
This makes the approach more suitable for real-world applications where data is not guaranteed to be perfectly sampled. As such neural SDEs will be used moving forward in this research.
