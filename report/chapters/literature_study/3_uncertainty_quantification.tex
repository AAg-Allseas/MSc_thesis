...
In the forward problem, \(f, \, x, \, y\) are known. \(z\) can be calculated. However, if \(f\) is an imperfect model or 
\(x,\, y\) are subject to some uncertainty, then this uncertainty will in some way affect \(z\). This is known as the 
...
Aleatoric uncertainty: inherent, irreducable randomness or noise in data. Eg. 
Epistemic uncertainty: Uncertainty due to unknowns. 
