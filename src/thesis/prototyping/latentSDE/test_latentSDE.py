...
def load_and_sample(
    ...
    zs = torch.zeros((len(ts), batch_size, latent_size), device=device)

    if network == "prior":
        if custom_sampling:
            eps = torch.randn(
                size=(batch_size, *latent_sde.pz0_mean.shape[1:]),
                device=latent_sde.pz0_mean.device,
            )
            ...
            with torch.no_grad():
                for i in range(1, ts.shape[0]):
                    zs[i] = (
                        zs[i - 1]
                        + latent_sde.h(ts[i], zs[i - 1]) * (ts[i] - ts[i - 1])
                        + latent_sde.g(ts[i], zs[i - 1]) * bm[i]
                    )
            xs = latent_sde.projector(zs)
        else:
            xs = latent_sde.sample(batch_size=batch_size, ts=ts, bm=bm, dt=timestep)

    elif network == "posterior":
        if custom_sampling:
            if data_sample == None:
                raise ValueError(
                    "data sample must be provided to sample posterior network"
                )

            with torch.no_grad():
                ctx = latent_sde.encoder(
                    torch.flip(data_sample, dims=(0,)).contiguous()
                )
                ...
                for i in range(1, ts.shape[0]):
                    zs[i] = (
                        zs[i - 1]
                        + latent_sde.f(ts[i], zs[i - 1]) * (ts[i] - ts[i - 1])
                        + latent_sde.g(ts[i], zs[i - 1]) * bm[i]
                    )
                xs = latent_sde.projector(zs)
        else:
            xs = latent_sde.sample_posterior(
                batch_size=batch_size, ts=ts, xs=data_sample, bm=bm, dt=timestep
            )
    else:
        raise ValueError(
            f"Incorrect network type: {network}. Must be either 'prior' or 'posterior'"
        )
...
def integrate_sde(
    sde: LatentSDE, ts: torch.Tensor, bm: torch.Tensor, z0: torch.Tensor
) -> None:
