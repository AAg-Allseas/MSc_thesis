...
def train(
    ...
    scales = np.array(
        [1, 1, 1, 1, 1, 1, 1 / 250, 1 / 250, 1 / 250, 1 / 250, 1 / 160, 1 / 160]
    )
    ...
    with torch.no_grad():
        ...
        for layer_idx in [2, 4]:
            latent_sde.f_net[layer_idx].weight[:] = latent_sde.h_net[
                layer_idx
            ].weight.clone()
            latent_sde.f_net[layer_idx].bias[:] = latent_sde.h_net[
                layer_idx
            ].bias.clone()
            ...
            for epoch in tqdm.trange(0, num_epochs):
                ...
                for i, batch in enumerate(dataloader):
                    ...
                    if not torch.isfinite(loss):
                        logger.warning(
                            f"Non-finite loss at epoch {epoch}, batch {i}. Skipping update."
                        )
                        optimizer.zero_grad()
                        continue
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        latent_sde.parameters(), max_norm=1.0
                    )
                    optimizer.step()
                ...
                logger.info(
                    f"Epoch {epoch:06d} | lr: {lr_now:.5f} | mean_loss: {mean_train_loss:.4f}"
                )
...
def test_sample(
    ...
    if bm is None:
        bm = torchsde.BrownianInterval(
            0, 10800, size=(batch_size, latent_size), dt=0.02, device=device
        )

    logger.info(
        f"Starting sampling - {datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}"
    )
    sample = latent_sde.sample(batch_size=batch_size, ts=ts, bm=bm)
    logger.info(
        f"Finished sampling - {datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}"
    )
    return sample
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning(
            f"Run interrupted {datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}"
        )
