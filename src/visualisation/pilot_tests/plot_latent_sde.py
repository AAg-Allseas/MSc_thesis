...
def plot_latentsde(dataset: ParquetDataset, 
                   ...
                   sample_posterior: Optional[tuple[torch.Tensor, torch.Tensor]]=None) -> None:
    ...
    if sample_posterior is not None:
        ...
        handles_gen_posterior = ax.plot(ts_posterior, data_posterior[..., 0], alpha=0.6, color="g")
...
if __name__ == "__main__":
    ...
    with Profile() as profile:
        sample_data_prior = load_and_sample(Path("dump/latentsde/proof_of_concept/latentsde_491.pth"),
                                        batch_size=5,
                                        data_size=12,
                                        latent_size=4,
                                        context_size=64,
        ...
        batched_data_states = torch.cat(dataset_states, dim=1)  # (time_steps, batch_size, features)
        sample_data_posterior = load_and_sample(Path("dump/latentsde/proof_of_concept/latentsde_491.pth"),
                                batch_size=5,
                                data_size=12,
                                latent_size=4,
                                context_size=64,
